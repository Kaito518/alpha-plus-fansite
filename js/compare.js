...
function resetCompare() {
  compareState.mbti = null;
  compareState.blood = null;
  compareState.from = null;
  compareState.height = null;

  document.querySelectorAll(".compare-filter-btn").forEach((b) => {
...
function initCompare() {
  ...
  const bloods = [
    ...new Set(MEMBERS.map((m) => m.blood.replace(/（.*?）/, "").trim())),
  ].sort();
  const froms = [...new Set(MEMBERS.map((m) => m.from))].sort();
  const heights = [
    ...new Set(MEMBERS.map((m) => heightCategory(m.height))),
  ].sort();
...
function buildFilterBtns(containerId, values, key) {
  ...
  values.forEach((v) => {
    ...
    btn.addEventListener("click", () => {
      ...
      wrap
        .querySelectorAll(".compare-filter-btn")
...
function renderCompareResult() {
  ...
  const activeFilters = Object.entries(compareState).filter(
    ([, v]) => v !== null,
  );
  ...
  if (!activeFilters.length) {
    label.textContent = "";
    grid.innerHTML =
      '<p style="font-size:13px;color:var(--sub);text-align:center;padding:1.5rem">上のフィルターを選ぶと共通点があるメンバーが表示されます♡</p>';
    return;
  }
  ...
  if (!matched.length) {
    grid.innerHTML =
      '<p style="font-size:13px;color:var(--sub);text-align:center;padding:1.5rem">条件に一致するメンバーがいません</p>';
    return;
  }
